  ...
  <div class="question ${(j % 2) == 0 ? 'odd' : 'even'}">
    ...
    <div class=answer>
      ...
      <g:each in="${question?.answers?.sort{it?.sortOrder}}" status="k" var="answer">
        <g:radio class="radio" name="question_${prefix}${question?.id}" value="${answer?.text}" checked="${answer?.defaultAnswer}"/>
        ${answer?.text}
        <br/>
      </g:each>
      <%
      ...
      %>
      <g:each in="${question?.answers?.sort{it?.sortOrder}}" status="k" var="answer">
        <g:checkBox class="checkbox" name="question_${prefix}${question?.id}" value="${answer?.text}" checked="${answer?.defaultAnswer}"/>
        ${answer?.text}
        <br/>
