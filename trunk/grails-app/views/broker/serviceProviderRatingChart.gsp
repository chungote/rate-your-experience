...
<div class="body">
  <h1>My Service Providers</h1>
  <g:if test="${flash.message}">
    <div class="message">${flash.message}</div>
  </g:if>
  <g:form action="serviceProviderRatingChart" method="post">
  <%--
    <g:formRemote name="chartForm" url="[action:'ratingChart']" update="ratingChart">
  --%>
    <g:hiddenField name="broker" value="$broker"/>
    <g:hiddenField name="id" value="$broker"/>
    <div class="dialog">
      <div class="criteria">
        ...
        <div class="chartFormElement" id="chartProviderType">
          <label for="providerType"><g:message code="responseSearch.providerType.label" default="Provider Type"/></label>
          <g:select name='providerType'
                  from='${providerTypeList}'
                  value="${providerType}"
                  noSelection="['':'All Providers']"/>
        </div>
        <div class="chartFormElement" id="chartButtons">
          <input type="submit" value="Chart It!"/>
        </div>
      </div>
      <div class="ratingChart" id="ratingChart">
