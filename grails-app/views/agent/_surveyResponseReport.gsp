    ...
    <table>
      ...
      <g:each in="${surveyResponseInstanceList}" status="i" var="surveyResponseInstance">
        ...
        <tr class="${(start % 2) == 0 ? 'odd' : 'even'}">
          ...
          <td><g:link controller="surveyResponse" action="show" id="${surveyResponseInstance.id}">${fieldValue(bean: surveyResponseInstance, field: "notification.listing.sellingAgent.firstName")}&nbsp;${fieldValue(bean: surveyResponseInstance, field: "notification.listing.sellingAgent.lastName")}</g:link></td>
          <td><g:link controller="surveyResponse" action="show" id="${surveyResponseInstance.id}">${fieldValue(bean: surveyResponseInstance, field: "notification.listing.buyerAgent.firstName")}&nbsp;${fieldValue(bean: surveyResponseInstance, field: "notification.listing.buyerAgent.lastName")}</g:link></td>
