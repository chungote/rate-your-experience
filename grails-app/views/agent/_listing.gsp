                    ...
                    <g:each in="${listingInstanceList}" status="i" var="listingInstance">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
			                   ...
			                   <g:set var='flowState' value="${listingInstance.transaction.notification?.flowState[0]}" />
					   							...
					   							<g:link controller="surveyInvitationRequest" action="edit" id="${listingInstance.transaction.id[0]}">Request<br/>Info<br/>from Agent</g:link></td>
					                                      		   </g:if>
					   						   	 ...
					   						   	 <g:link controller="surveyInvitationRequest" action="show" id="${listingInstance.transaction.notification.surveyInvitationRequest.id[0]}">Survey<br/>Request<br/>Sent to<br/>Client</g:link>
					   						    ...
					   						    <g:if test="${flowState==NotificationFlowState.SURVEY_REVIEWED_BY_AGENT}">
					   							 <g:link controller="surveyResponse" action="edit" id="${listingInstance.transaction.notification.surveyResponse.id[0]}">Survey<br/>Reviewed<br/>by Agent</g:link>
					   						   </g:if>
