  ...
  <div class="list">
    <richui:tabView id="tabView">
      ...
      <richui:tabContents>
        ...
        <g:each status="i" var="type" in="${serviceProviderList}">
          <richui:tabContent>
            <g:render template="showGroup" model="['responses':surveyResponseInstance?.responses, 'group':surveyResponseInstance?.survey?.groups?.find{it.type == type}, 'type':type,'prefix':'groups['+i+'].']"/>
          </richui:tabContent>
        </g:each>

        <richui:tabContent>
          <div class="sRebuttal">
            <g:fieldValue bean="${surveyResponseInstance}" field="rebuttal"/>
          </div>
        </richui:tabContent>
